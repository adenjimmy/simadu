...
<section class="content">
     <div class="box">
        ...
        <li class="active"><a href="<?php echo site_url('rekening/rek7');?>">Rekening 7</a></li>
             ...
             <div class="col-sm-12">
              <table id="example1" class="table table-bordered table-striped" role="grid" aria-describedby="example1">               
                ...
                <tr>
                  ...
                  <th>Nama Rekening Level 7</th>
                  <th>Keterangan Rekening Level 7</th>                  
                  <th>Action</th>
               </tr>
                </thead>
                <tbody>
                    <?php $no=1; foreach($rek7 as $row){ ?>
                <tr>
                    <td><?php echo $no; ?></td>
                    <td><?php echo $row->dien006_01; ?>. <?php echo $row->dien007_01; ?>. <?php echo $row->dien008_01; ?>. <?php echo $row->dien009_01; ?>.
                    	 <?php echo $row->dien010_01; ?>. <?php echo $row->dien011_01; ?>. <?php echo $row->dien012_01; ?></td>
                    
                    <td><?php echo $row->dien012_02; ?></td>
                    <td><?php echo $row->dien012_03; ?></td>
                    <td>
...
<script type="text/javascript">
 var save_method; //for save method string
    ...
    function edit_bid(id)
    {
      ...
      $.ajax({
        url : "<?php echo site_url('rekening/edit_rek7/')?>/" + id,
        type: "GET",
        dataType: "JSON",
        success: function(data)
        {
            ...
            $('[name="kd7"]').val(data.dien012_02);
            $('[name="kd8"]').val(data.dien012_03);            
    ...
    function save()
    {
      ...
      if(save_method == 'add')
      {
          url = "<?php echo site_url('rekening/tambah_rek7')?>";
      }
      else
      {
        url = "<?php echo site_url('rekening/update_rek7')?>";
      }
    ...
    function delete_bid(id)
    {
      if(confirm('Are you sure delete this data?'))
      {
        // ajax delete data from database
          $.ajax({
            url : "<?php echo site_url('rekening/delete_rek7/')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
        ...
        <div class="modal modal-info" id="modal_form" role="dialog">
          ...
          <input type="hidden" value="" name="id"/>
            ...
            <div class="form-group">
              ...
              <div class="col-md-9">
                <input name="kd6" readonly="" value=<?=$kodeunik;?> class="form-control " type="number" >
                <span class="help-block"></span>
              </div>
            </div>               
            <div class="form-group">
              <label class="control-label col-md-3">Rekening Level 7</label>
              <div class="col-md-9">
                <input name="kd7" required="" placeholder="Input Nama Rekening" class="form-control" type="text" >
                <span class="help-block"></span>
              </div>
            </div>
            <div class="form-group">
              <label class="control-label col-md-3">Deskrpsi Level 7</label>
              <div class="col-md-9">
                <input name="kd8" required="" placeholder="Input Desk Rekening" class="form-control" type="text" >
                <span class="help-block"></span>
              </div>
            </div>                           
       		</div>      
